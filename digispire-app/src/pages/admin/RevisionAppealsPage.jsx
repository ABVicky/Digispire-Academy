...
import {
  History, User, Calendar, Check, Trash2, X, MessageSquare, Search,
  CheckCircle2, BookOpen, BookMarked, Clock, Filter
} from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100 flex items-center gap-4 hover:shadow-md transition">
      ...
      <div className="bg-white p-4 rounded-3xl border border-slate-100 flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 text-xs bg-slate-50 hover:bg-slate-100/70 border border-slate-200 focus:border-[#255A84] rounded-2xl outline-none transition"
          />
        </div>
      </div>
      ...
      {filteredAppeals.length === 0 ? (
        <div className="text-center py-16 text-slate-400 bg-white rounded-[2.5rem] border border-dashed border-slate-200 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredAppeals.map(appeal => (
            <div
              key={appeal.id}
              className={`bg-white rounded-[2rem] border p-6 flex flex-col justify-between transition-all duration-300 hover:shadow-md hover:border-slate-200 ${appeal.status === 'pending' ? 'border-amber-100 ring-2 ring-amber-500/5' : 'border-slate-100'}`}
            >
              {/* Card Top / Metadata */}
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        <span className="text-[9px] font-bold text-slate-400 uppercase font-mono tracking-wider">{appeal.studentId}</span>
                        <span className="text-slate-200">·</span>
                        <span className="text-[9px] font-bold text-[#255A84] uppercase tracking-wider">{appeal.studentBatch} Batch</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-1.5">
                    {appeal.status === 'pending' ? (
                      <span className="text-[9px] font-bold text-amber-700 bg-amber-50 border border-amber-100 px-2.5 py-1 rounded-xl flex items-center gap-1 animate-pulse">
                        <Clock size={8} /> Pending
                      </span>
                    ) : (
                      <span className="text-[9px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-100 px-2.5 py-1 rounded-xl flex items-center gap-1">
                ...
                {appeal.notes ? (
                  ...
                  <p className="text-[10px] text-slate-400 italic">No notes provided by student.</p>
                )}
      ...
      {resolveModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl border border-slate-100 w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleResolve} className="p-6 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 border border-slate-200 rounded-2xl text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-none bg-slate-50/50"
                  rows={4}
                  placeholder="Summarize the revision session, next steps, or resources provided..."
                />
              </div>
