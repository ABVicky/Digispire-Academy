    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button 
          ...
          className="px-6 py-3 bg-[#255A84] text-white rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg shadow-[#255A84]/20 flex items-center gap-2 hover:scale-105 transition-all"
        >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {tools.map(tool => {
            ...
            return (
              <div key={tool.id} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 flex flex-col group">
                ...
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{cat.label}</p>
                ...
                <a href={tool.url} target="_blank" rel="noopener noreferrer" className="mt-5 flex items-center justify-between p-3 bg-slate-50 rounded-xl group/link">
                  <span className="text-[11px] font-bold text-slate-400 truncate max-w-[150px]">{new URL(tool.url).hostname}</span>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-lg shadow-2xl p-8 animate-in slide-in-from-bottom duration-300">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Tool Name</label>
                <input required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="e.g. Google Analytics 4" className="w-full px-5 py-4 bg-slate-50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#255A84] transition-all" />
              </div>

              <div>
                <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Website URL</label>
                <input required type="url" value={form.url} onChange={e => setForm({ ...form, url: e.target.value })} placeholder="https://..." className="w-full px-5 py-4 bg-slate-50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#255A84] transition-all" />
              </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {CATEGORIES.map(c => (
                    <button
                      ...
                      className={`flex items-center gap-2 p-3 rounded-xl border transition-all ${form.categoryId === c.id ? 'bg-[#255A84] text-white border-transparent shadow-lg' : 'bg-slate-50 text-slate-500 border-transparent hover:bg-slate-100'}`}
                    >
                      <c.icon size={16} />
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">Short Description</label>
                <textarea required value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows="3" placeholder="What is this tool used for?" className="w-full px-5 py-4 bg-slate-50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#255A84] transition-all resize-none" />
              </div>

              <button disabled={saving} type="submit" className="w-full py-5 bg-[#255A84] text-white rounded-xl font-bold text-sm uppercase tracking-widest shadow-xl shadow-[#255A84]/20 active:scale-95 transition-all flex items-center justify-center gap-3">
